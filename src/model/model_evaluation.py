...
# -------------------------------------------------------------------------------------
# Production tracking setup — NO dagshub.init() (triggers OAuth browser prompt in CI)
# -------------------------------------------------------------------------------------
    ...
    with mlflow.start_run() as run:
        try:
            ...
            try:
                ...
                mlflow.log_param(
                    "best_model_type",
                    selection.get("best_model", "unknown")
                )
                mlflow.log_metric(
                    "cv_auc",
                    selection.get("cv_auc", 0.0)
                )

            except FileNotFoundError:
                logging.warning(
                    "model_selection.json not found — skipping selection metadata"
                )
